...
| `hacode.infra.k8s_addons`    | Opinionated k8s addons (Headlamp web UI via Helm) installed from the controller against an existing cluster. Long-lived admin token exported to disk; each addon gated by its own `_enabled` flag.           |
