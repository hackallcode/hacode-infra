...
| `hacode.infra.k8s_addons`    | Opinionated k8s addons (Cilium CNI, Headlamp web UI, Ingress-NGINX) installed from the controller against an existing cluster via Helm. Long-lived Headlamp admin token exported to disk; each addon gated by its own `_enabled` flag. |
